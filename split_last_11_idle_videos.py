...
def main():
    ...
    output_dir = "data/exercises/right-knee-to-90-degrees/idle_segments_last11"
    ...
    video_files.sort()  # Sort to get consistent last 11
    print(f"Found {len(video_files)} video files")
    
    # Take last 11 videos
    last_11_videos = video_files[-11:]
    print(f"Processing last 11 videos:")
    for i, video in enumerate(last_11_videos):
    ...
    for i, video_file in enumerate(last_11_videos):
        print(f"\n{'='*50}")
        print(f"Processing video {i+1}/11: {Path(video_file).name}")
        print(f"{'='*50}")
