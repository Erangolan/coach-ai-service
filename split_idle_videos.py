...
def main():
    ...
    output_dir = "data/exercises/right-knee-to-90-degrees/idle_segments"
    ...
    video_files.sort()  # Sort to get consistent first 20
    print(f"Found {len(video_files)} video files")
    
    # Take first 20 videos
    first_20_videos = video_files[:20]
    print(f"Processing first 20 videos:")
    ...
    for i, video_file in enumerate(first_20_videos):
        print(f"\n{'='*50}")
        print(f"Processing video {i+1}/20: {Path(video_file).name}")
        print(f"{'='*50}")
